...
from code_scripts.visualization_extra import _input_validation, max_rulelength_visual
from code_scripts.visualization_extra import define_relative_position, plot_arrow
...
def plot_rules(rules, preds, baselines, weights, max_rulelen=None,
    ...
    if preds_distr is not None:
        ...
        if len(weights) == 3 and weights[0] > 0.484 and weights[1] > 0.344:
            print(' Here we are')
    ...
    for j, pos in zip(range(n_cols), pos_list):
        ...
        # TODO: make class out of all this so that plot_arrow can 
        # be a function that can inherit  attribtues from outside
    ...
    final_pred_str += " (= " + " + ".join([
        rf"{preds[i][-1]:.{round_digits}f}$\times${weights[i]:.2f}" 
