...
def plot_rules(
    ...
    if preds_distr is not None:
        ...
        x = np.linspace(preds_distr.min() - 0.00 * extent, preds_distr.max() + 0.00 * extent, 100)
    ...
    if preds_distr is None:  # no extra axis objects for density plot
        fig, aaxs = plt.subplots(
            figsize=(5 * nrules + 2, plot_height_rulebased + 2),
            nrows=2,
            ncols=nrules,
            sharey=True,
        ...
        fig, aaxs = plt.subplots(
            figsize=(5 * nrules + 2, plot_height_rulebased + 3),
            nrows=3,
            ncols=nrules,
            sharey="row",
    ...
    cbar = plt.colorbar(
        ...
        pad=0.04,
        aspect=aspect,
        label="Change w.r.t. baseline",
    )
    cbar.ax.tick_params(labelsize=base_fontsize + 1)
    cbar.set_label(
        "Change w.r.t. baseline", fontsize=base_fontsize + 1
    )  # Adjusting the main label size (again)
        ...
        for bsl, ax in zip(baselines, rule_axs):
            for pred in other_preds:
                ax.plot(
                    ...
                    zorder=-500,
                    lw=0.6,
                )  # zorder compared to zorder in rule_axs
    ...
    for bsl, rule, pred, ax in zip(baselines, rules, preds, rule_axs):
        ...
        for j, _ in enumerate(rule):
            ...
            # xmin, xmax = ax.get_xlim()
            xtext = (
                2 * traj[j] + 2 * traj[j + 1] + 6 * (xmax + xmin) / 2
            ) / 10  # traj[0] originally
    ...
    final_pred_str += (
        ...
        + " + ".join(
            [
                rf"{frmt_pretty_print(preds[i][-1], tot_digits)}" rf"$\times${weights[i]:.2f}"
    ...
    if b_box_pred is not None:
        ypos_fin_text = 0.05
        bbox_pred_str = frmt_pretty_print(
            b_box_pred, tot_digits
        )  # works both with single values and multi-output
    ...
    fig.text(
        0.5, ypos_fin_text, final_pred_str + bbox_pred_str, fontsize=base_fontsize + 2, ha="center"
    )
...
def read_rules(file, file_extra=None):
    ...
    for line in btrex_rules:
        ...
        if "Baseline prediction" in line:
            ...
            f_baselines.append(
                [float(value.strip()) for value in baseline_values]
            )  # Append as a list
            rule = []
            pred = []
        if "node" in line:
