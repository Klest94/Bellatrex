...
def pack_trained_ensemble(
    clf: object, set_up: str = "auto", time_to_bin: float | None = None
) -> dict:
    """
    ...
    if time_to_bin is not None and not isinstance(clf, RandomSurvivalForest):
        warnings.warn(
            f"time_to_bin parameter is ignored for any estimator other than RandomSurvivalForest.\n"
            f"Found: {type(clf)}"
        )
...
def tree_to_dict(clf_obj, idx, output_format, time_to_bin=None):
    ...
    if isinstance(tree_obj, SurvivalTree):  # case SETUP=survival
        ...
        if (
            time_to_bin is None and tree_dict["unique_times_"] is not None
        ):  # select median time interval
        ...
        if output_format == "predict_survival_curve":
            ...
            tree_dict["values"] = np.sum(
                tree.value[:, tree_dict["is_event_time_"], 0], axis=1
            ).reshape(-1, 1)
    ...
    # case SETUP=binary:
        ...
        raise ValueError(
            f"Combination of learner '{tree_obj.__class__.__name__}' and"
