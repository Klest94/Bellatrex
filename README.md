...
Bellatrex is a novel explainability toolbox that is designed to explain Random Forest predictions. It does so by Building Explanations through a LocalLy AccuraTe Rule EXtractor (hence the name: Bellatrex) for a given test instance, with only a few diverse rules.

Starting from the decision trees generated by a random forest, Bellatrex 1) pre-selects a subset of the rules used to make the prediction, 2) creates a vector representation of such rules, 3) projects them to a low-dimensional space, 4) clusters such representations to pick a rule from each cluster to explain the instance prediction. One rule per cluster is shown to the end user through easy to read plots, and a GUI let the user explore similar rules to the ones extracted.

Another strength of Bellatrex is that it can handle many different prediction tasks within Random Forests in `scikit-learn`: in addition to the binary classification task with `RandomForestClassifier`, Bellatrex can generate explanations for multi-label predictions from the same learner class, as well as single- or multi-output regression tasks with `RandomForestRegressor`. Finally, Bellatrex is compatible with `scikit-survival`'s `RandomSurvivalForest` for time-to-event prediction in the presence of right censored data.
