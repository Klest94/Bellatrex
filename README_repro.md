...
This branch is dedicated to reproducing the original results from [our published work](https://ieeexplore.ieee.org/abstract/document/10105927) on Bellatrex.
Bellatrex tackles the challenge of interpreting Random Forest models by generating easy-to-understand explanations and insights into how predictions are made. The tool achieves this by Building Explanations through a LocalLy AccuraTe Rule EXtractor (hence the name: Bellatrex) for given test instances, extracting only a few diverse rules.

For the latest developments and updates, please refer to the [main branch](https://github.com/Klest94/Bellatrex/tree/main).
...
Here we share details about the datasets in use for the comparison. In particular, we share information such as number of instances ``n`` and number of covariates ``p``.

Additionally, we report:
- the prevalence of the positive class for binary classification data;
- the frequency of the censoring effect for time-to-event (survival) data
- the number of output labels (or classes) for multi-label classification and multi-target regression.

The pre-processed and post-processed datasets can be found in the [``/datasets``](https://github.com/Klest94/Bellatrex/tree/archive/reproduce-Dedja2023/datasets) folder.
...
The procedure is performed separately for the five predictive setups, and the statistical significance of the results is evaluated through the Friedmnan-Nemenyi post-hoc test.
